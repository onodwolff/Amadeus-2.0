...
import { NodeMetricPoint } from '../../../api/models';

export interface MetricsChartSeries {
  readonly id: string;
  readonly name: string;
  readonly color: string;
  readonly data: readonly NodeMetricPoint[];
}
...
export class MetricsChartComponent implements AfterViewInit, OnChanges, OnDestroy {
  @Input() series: readonly MetricsChartSeries[] = [];
  ...
  readonly trackById = (_: number, item: MetricsChartSeries) => item.id;
  ...
  private transformData(data: readonly NodeMetricPoint[]): LineData[] {
    return data
