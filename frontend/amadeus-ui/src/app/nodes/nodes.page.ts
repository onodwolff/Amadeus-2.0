...
export class NodesPage implements OnInit, OnDestroy {
  ...
  private loadNodeDetail(nodeId: string): void {
    ...
    this.nodesApi.getNodeDetail(nodeId).subscribe({
      next: (detail) => {
        ...
        const lifecycle = this.normalizeLifecycle(detail?.lifecycle);
        this.nodeLifecycle.set(lifecycle);
        this.patchNodeFromDetail(detail?.node);
        this.detailError.set(null);
      },
      error: (err) => {
        console.error(err);
  ...
  private loadNodeLogs(nodeId: string): void {
    ...
    this.nodesApi.getNodeLogs(nodeId).subscribe({
      next: (payload) => {
        this.nodeLogs.set(this.normalizeLogs(payload?.logs));
        this.logsError.set(null);
      },
      error: (err) => {
        console.error(err);
  ...
  private connectNodeStream(nodeId: string): void {
    ...
    this.logStreamSubscription = events$.subscribe({
      next: (payload) => {
        this.nodeLogs.set(this.normalizeLogs(payload?.logs));
        this.nodeLifecycle.set(this.normalizeLifecycle(payload?.lifecycle));
      },
      error: (err) => {
        console.error(err);
  ...
  private patchNodeFromDetail(detailNode: NodeHandle | undefined | null): void {
    if (!detailNode) {
      return;
    }
    this.nodes.update((nodes) =>
      nodes.map((node) => (node.id === detailNode.id ? { ...node, ...detailNode } : node)),
    );
  }

  private normalizeLogs(logs: NodeLogEntry[] | null | undefined): NodeLogEntry[] {
    if (!Array.isArray(logs)) {
      return [];
    }
    return [...logs].sort((a, b) => this.toTimestamp(a.timestamp) - this.toTimestamp(b.timestamp));
  }

  private normalizeLifecycle(
    lifecycle: NodeLifecycleEvent[] | null | undefined,
  ): NodeLifecycleEvent[] {
    if (!Array.isArray(lifecycle)) {
      return [];
    }
    return [...lifecycle].sort(
      (a, b) => this.toTimestamp(a.timestamp) - this.toTimestamp(b.timestamp),
    );
  }
