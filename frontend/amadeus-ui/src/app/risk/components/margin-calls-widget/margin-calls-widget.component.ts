...
import { observeRiskMarginCalls } from '../../../ws';
import { WsConnectionState, WsService } from '../../../ws.service';

@Component({
  standalone: true,
  selector: 'app-risk-margin-calls-widget',
  imports: [CommonModule, DatePipe],
  templateUrl: './margin-calls-widget.component.html',
  styleUrls: ['../risk-alert-widget/risk-alert-widget.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class RiskMarginCallsWidgetComponent {
  private readonly ws = inject(WsService);
  private readonly api = inject(RiskApi);
  private readonly notifications = inject(NotificationService);

  private readonly stream = observeRiskMarginCalls(this.ws);
  ...
  acknowledge(alert: RiskAlert): void {
    ...
    this.api
      ...
      .subscribe({
        next: () => {
          this.notifications.success('Margin call acknowledged.', 'Risk alerts');
        },
        error: () => {
          this.notifications.error('Failed to acknowledge margin call.', 'Risk alerts');
        },
      });
  }

  escalate(alert: RiskAlert): void {
    if (!alert || !alert.escalatable || alert.escalated) {
      return;
    }
    const key = this.actionKey(alert.id, 'escalate');
    this.setPending(key, true);
    this.api
      .escalateMarginCall(alert.id)
      .pipe(finalize(() => this.setPending(key, false)))
      .subscribe({
        next: () => {
          this.notifications.info('Margin call escalated to treasury.', 'Risk alerts');
        },
        error: () => {
          this.notifications.error('Unable to escalate margin call.', 'Risk alerts');
        },
      });
  }

  isActionPending(alert: RiskAlert, action: 'ack' | 'escalate'): boolean {
