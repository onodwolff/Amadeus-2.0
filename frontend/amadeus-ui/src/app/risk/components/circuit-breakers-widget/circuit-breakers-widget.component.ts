...
import { observeRiskCircuitBreakers } from '../../../ws';
import { WsConnectionState, WsService } from '../../../ws.service';

@Component({
  standalone: true,
  selector: 'app-risk-circuit-breakers-widget',
  imports: [CommonModule, DatePipe],
  templateUrl: './circuit-breakers-widget.component.html',
  styleUrls: ['../risk-alert-widget/risk-alert-widget.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class RiskCircuitBreakersWidgetComponent {
  private readonly ws = inject(WsService);
  private readonly api = inject(RiskApi);
  private readonly notifications = inject(NotificationService);

  private readonly stream = observeRiskCircuitBreakers(this.ws);
  ...
  acknowledge(alert: RiskAlert): void {
    ...
    this.api
      ...
      .subscribe({
        next: () => {
          this.notifications.success('Circuit breaker acknowledged.', 'Risk alerts');
        },
        error: () => {
          this.notifications.error('Failed to acknowledge circuit breaker.', 'Risk alerts');
        },
      });
  }

  unlock(alert: RiskAlert): void {
    if (!alert || !alert.unlockable || !alert.locked) {
      return;
    }
    const key = this.actionKey(alert.id, 'unlock');
    this.setPending(key, true);
    this.api
      .unlockCircuitBreaker(alert.id)
      .pipe(finalize(() => this.setPending(key, false)))
      .subscribe({
        next: () => {
          this.notifications.success('Circuit breaker unlocked.', 'Risk alerts');
        },
        error: () => {
          this.notifications.error('Unable to unlock circuit breaker.', 'Risk alerts');
        },
      });
  }

  isActionPending(alert: RiskAlert, action: 'ack' | 'unlock'): boolean {
